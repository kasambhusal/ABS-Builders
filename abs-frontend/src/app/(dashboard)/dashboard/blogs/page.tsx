...
export default async function DashboardBlogsPage() {
  ...
  const { rows: blogs, error } = await listPrivateBlogs().then(
    ...
    (e: unknown) => ({
      ...
      error:
        e instanceof ApiError
          ? e.message
          : "Unable to load blogs. Check API configuration.",
    }),
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-stone-900">Blogs</h1>
          <p className="text-sm text-stone-600">
            Articles and updates for the marketing site.
          </p>
        </div>
        <Link
